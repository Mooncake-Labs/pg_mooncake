...
extern "C-unwind" fn mooncake_scan_table_begin(
    ...
    let mut bytes = block_on(scan_table_begin(&mut *get_stream(), schema, table, lsn))
...
extern "C-unwind" fn mooncake_scan_table_end(
    ...
    let schema = ptr_to_str(schema).to_owned();
    let table = ptr_to_str(table).to_owned();
    block_on(scan_table_end(&mut *get_stream(), schema, table)).expect("scan_table_end failed");
}
