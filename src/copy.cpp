 ...
 */
static duckdb::string CreateRelationCopyString(ParseState *pstate, CopyStmt *copy_stmt, bool *allowed) {
...
static duckdb::string CreateCopyOptions(CopyStmt *copy_stmt, bool *options_valid) {
...
bool DuckdbCopy(PlannedStmt *pstmt, const char *query_string, struct QueryEnvironment *query_env, uint64 *processed) {
    ...
    if (copy_stmt->query) {
        ...
        rewritten_query_string =
            duckdb::StringUtil::Format("COPY (%s) %s %s %s", pgduckdb_get_querydef(query, false /*pretty*/), direction,
    ...
    if (res->HasError()) {
        elog(ERROR, "(pg_mooncake/ExecuteQuery) %s", res->GetError().c_str());
    }
