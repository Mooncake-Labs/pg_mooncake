...
public:
    explicit ColumnstoreDeleteGlobalState() {}
...
unique_ptr<PhysicalOperator> PostgresCatalog::PlanDelete(ClientContext &context, LogicalDelete &op,
    ...
    auto del = make_uniq<ColumnstoreDelete>(op.types, op.estimated_cardinality,
                                            op.table.Cast<ColumnstoreTableCatalogEntry>().GetTable(), bound_ref.index);
