...
#include "utils/resize_uninitialized.hpp"
...
struct CacheReadChunk {
    ...
    // Acts as intermediate buffers take bytes read from remote storage, and copy to requested memory address.
    ...
    //
    // TODO(hjiang): For middle chunks, the performance could be improved further for cache miss case: remote IO
...
void CacheLocal(const CacheReadChunk &chunk, FileSystem &local_filesystem, const FileHandle &handle,
    ...
    const auto local_temp_file = StringUtil::Format("%s%s.%s.httpfs_local_cache.tmp", x_mooncake_local_cache, fname,
                                                    UUID::ToString(UUID::GenerateRandomUUID()));
    {
        ...
        local_filesystem.Write(*file_handle, const_cast<char *>(chunk.content.data()),
                               /*nr_bytes=*/chunk.content.length(), /*location=*/0);
        file_handle->Sync();
    }
...
void ColumnstoreReadCacheFileSystem::ReadAndCache(FileHandle &handle, char *buffer, idx_t requested_start_offset,
    ...
    const idx_t num_io_op = (aligned_last_chunk_offset - aligned_start_offset) / block_size + 1;
    ...
    for (idx_t io_start_offset = aligned_start_offset; io_start_offset <= aligned_last_chunk_offset;
        ...
        // Implementation-wise, middle chunks are easy to handle -- read in [block_size] bytes, and copy the whole chunk
        // to the requested memory address; but the first and last chunk require special handling.
